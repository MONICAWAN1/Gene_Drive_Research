...
def loadGres(currH, gdFile):
    ...
    '''
    return load_pickle(f"h{currH}_allgdres{gdFile}G.pickle")
...
def opt_r(F, grad_F, vars, s, c, h, params, gdFile, alpha=1e-4, beta=0.8, r_init=1.0):
    r = r_init
    while True:
...
def hap_grid_mapping(params, gdFile):
    ...
    for (s, c, h) in gd_configs:
        ...
        # if ((s, c, h), 1.0) in stabilityRes['Fixation'] and math.isclose(s, 0.01) and math.isclose(c, 0.01):
            gd_curve = gd_res[(s, c, h)]['q']
            best_diff = 10000
            best_s = None
            best_h = h
